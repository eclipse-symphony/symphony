...
1. Define your application as a Symphony `solutionversion` object. This object contains four components: a Docker container that is to be deployed on Kubernetes, a UWP application package that is to be deployed to Windows, a Web Assembly module that is to be deployed on a Flatcar VM, and a ThreadX binary image that is to be deployed on a MIMXRT1170-EVK board.
...
| [solutionversion.yaml](../../samples/multi-os/solutionversion.yaml) | SolutionVersion definition |
...
2. Build the solutionversion for `x64` architecture, `Debug` profile.
...
3. On the `solutionversion-manager` configuration, you should define local target providers you want to use. **Note in current version, the provider name needs to match with the target name**.

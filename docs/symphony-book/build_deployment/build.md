...
The startup symphony-k8s pod yaml is generated by `mage helmTemplateController`.

The reason why we want to split these two is we want **helm** to render the final symphony-k8s pod yaml according to helm values during runtime, so we manually modify the `../packages/helm/symphony/templates/symphony-core/symphony-k8s-controller.yaml` with helm value expressions.

> **IMPORTANT**: If you have any symphony-k8s pod yaml changes, please rerun `mage helmTemplateController` and compare the generated startup yaml with existing yaml, **re-apply the helm expression** accordingly.
...
# mage helmTemplateController
# Generated startup yaml will be updated in  ../packages/helm/symphony/templates/symphony-core/symphonyk8s-controller.yaml, developers need to preserve the helm expressions.
```

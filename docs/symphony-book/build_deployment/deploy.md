...
Symphony's [job manager](../managers/_overview.md) invokes Symphony's reconcile API on the solutionversion vendor through HTTP. Make sure that your job manager is configured to talk to the solutionversion vendor host FDN (or load-balancer FDN) instead of `localhost`.

### State stores

Most Symphony components are stateless, with exception of the [instance manager](../managers/_overview.md). The instance manager uses a state store to remember the last deployment it has successfully applied. When you have multiple instance managers running (by scaling out the solutionversion vendor), they need to use a shared state store instead of the in-memory state store.

...
Symphony separates configuration management and configuration serving. Before configurations are served to applications, Symphony performs all necessary resolutionversions, such as resolving inheritances and filling contextual information.

Configurations can be injected into solutionversion components as environment variables or file mounts (on Kubernetes). An application can also query Symphony REST API to retrieve configurations.
...
1. Symphony creates a Kubernetes ConfigMap named `app-config`. The ConfigMap contains an `appSettings.json` file, whose content is populated from a `config-obj` catalogversion object.
...
To query a configuration object, you can send an authenticated **GET** request to the /`catalogversions/registry` route.

### Query by id

Send a **GET** request to `catalogversions/registry/<id>`. You'll get the complete configuration object (as a CatalogVersion object).
...
```bash
GET /catalogversions/registry/<id>?filterType=label&filterValue=app==my-app,shit==night
```
...
Symphony query APIs allows object projection using a Json Path syntax. Instead of retrieving the entire catalogversion object, you can directly retrieve specific portions or specific fields from a configuration object. 
For example, the following query returns only the ```foo``` field of a configuration object:
```bash
GET /catalogversions/registry/<id>?doc-type=yaml&path=$.spec.properties.foo
```
See [projection](../api/projection.md) for more details on query projection.

## Configuration resolutionversion
...
The default configuration provider shipped with Symphony is based on CatalogVersions objects. However, other providers can be added to resolve configurations from other sources and to provide additional functionality. For example, a caching provider can be added at the top of the precedence list to provide faster lookups, so the configurations don’t need to be repeatedly resolved. 

...
* A single solutionversion

  The easiest way to model your application is to use a single `solutionversion` object that has three components: `frontend`, `backend(v1)`, and `ingress`. When you are ready to do a canary deployment of `backend(v2)`, you add the `backend(v2)` as a new component to your application and reconfigure the `ingress`.

* Two solutionversions

  You can use two separate `solutionversion` objects to represent the frontend and the backend. This option may be preferred if the frontend and the backend are managed by different teams. The frontend contains just the frontend component, and the backend contains the `backend(v1)` component and an `ingress` configuration. When you need to deploy a new backend version, you add a `backend(v2)` to your backend solutionversion and adjust your `ingress` settings.

* SolutionVersions and a target

  You can move the `ingress` component to a `target` object, especially when the ingress is considered IT infrastructure and managed by IT pros instead of developers. In such cases, you can model your application with either a single `solutionversion` or two `solutionversions` as above and keep the `ingress` component in the `target` object.
...
4.	If the canary deployment succeeded, remove backend(v1) from the solutionversion and the workflow stops.

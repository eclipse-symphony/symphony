...
interface SolutionVersionCardProps {
...
function SolutionVersionCard(props: SolutionVersionCardProps) {
        ...
        <Card radius='none' shadow='lg' className='card'
            ...
            <CardHeader className="absolute z-10 top-0 flex-col !items-start bg-black/10">
                <div className="card_title">
                    {solutionversion.metadata.name.replace(`${solutionversion.metadata.labels.rootResource}-v-`, `${solutionversion.metadata.labels.rootResource}: `)}
                </div>
               {isHovered && (
            ...
            <CardBody className='absolute top-[80px] h-full bg-white/70'>    
                {activeView == 'properties' && (
                    <SolutionVersionSpecCard solutionversion={solutionversion.spec} />
                )}
                {activeView == 'topology' && (                    
...
export default SolutionVersionCard;
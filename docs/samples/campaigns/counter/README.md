# Counter CampaignVersion Sample
...
curl -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d @'./campaign.json' http://localhost:8080/v1alpha2/campaigns/counter-campaign
curl -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d @'./campaignversion.json' http://localhost:8080/v1alpha2/campaignversions/counter-campaign-v-version1

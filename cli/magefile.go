...
func BuildCli() error {
	if err := shellcmd.RunAll(
		...
		"CGO_ENABLED=0 GOARCH=arm GOARM=7 go build -o maestro-arm7",
...
func BuildApi() error {
	...
	if err := shellcmd.RunAll(
		...
		shellcmd.Command("CGO_ENABLED=0 GOARCH=arm GOARM=7 go build -o symphony-api-arm7"),
...
func GeneratePackages(des string) error {
	...
	if err := removePakcageIfExist(fmt.Sprintf("%s/maestro_linux_arm7.tar.gz", des)); err != nil {
		return err
	}
	...
	if err := shellcmd.RunAll(
		...
		shellcmd.Command(fmt.Sprintf("cp %s/api/symphony-api-arm7 %s", symphonyPath, des)),
		...
		shellcmd.Command(fmt.Sprintf("cp %s/cli/maestro-arm7 %s", symphonyPath, des)),
	...
	// package arm64
	arm7Command := fmt.Sprintf("tar -czvf maestro_linux_arm7.tar.gz maestro symphony-api symphony-api-no-k8s.json samples.json k8s iot-edge")
	if err := shellcmd.RunAll(
		shellcmd.Command(fmt.Sprintf("rm maestro")),
		shellcmd.Command(fmt.Sprintf("rm symphony-api")),
		shellcmd.Command(fmt.Sprintf("mv maestro-arm7 maestro")),
		shellcmd.Command(fmt.Sprintf("mv symphony-api-arm7 symphony-api")),

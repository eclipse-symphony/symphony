...
var _ = Describe("Create resources with sequential changes", Focus, func() {
	...
	var specTimeout = 30 * time.Second
	...
	DescribeTable("when performing create/update operations", Ordered, runner,
		...
		Entry(
			...
			TestCase{
				...
				Expectation: expectations.All(
					...
					successfullInstanceExpectation,
					helm.MustNew("simple-chart-1", "default", helm.WithReleaseCondition(helm.DeployedCondition)), // make sure the release is successfully deployed
				),
			},
		),
		...
		// Entry(
		// 	"it should fail the solution when component is invalid", SpecTimeout(specTimeout),
		// 	TestCase{

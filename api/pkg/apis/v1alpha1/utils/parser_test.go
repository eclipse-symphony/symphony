...
func TestParamsSingleArg(t *testing.T) {
	parser := NewParser("$config(abc)")
	node := parser.expr()
	_, err := node.Eval(nil)
	assert.NotNil(t, err)
}
func TestParamsNoProvider(t *testing.T) {
...
func TestParams(t *testing.T) {
	...
	parser := NewParser("$config(abc,def)")
	node := parser.expr()
	val, err := node.Eval(provider)
	assert.Nil(t, err)
	assert.Equal(t, "abc::def", val)
}
func TestParamsWithExpression(t *testing.T) {
	//create mock config provider
	provider := &mock.MockConfigProvider{}
	err := provider.Init(mock.MockConfigProviderConfig{})
	assert.Nil(t, err)
...
func TestParamsRecursiveMixed(t *testing.T) {
	//create mock config provider
	provider := &mock.MockConfigProvider{}
	err := provider.Init(mock.MockConfigProviderConfig{})
	assert.Nil(t, err)

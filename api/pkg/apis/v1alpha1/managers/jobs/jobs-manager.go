...
func (s *JobsManager) HandleHeartBeatEvent(ctx context.Context, event v1alpha2.Event) error {
	...
	log.Debugf(" M (Job): handling heartbeat h_%s, %v, %v", heartbeat.JobId, heartbeat.Action, heartbeat.JobAction)
	if heartbeat.JobAction == v1alpha2.JobUpdate {
		log.Debugf(" M (Job): update heartbeat h_%s, %v, %v", heartbeat.JobId)
		...
		log.Debugf(" M (Job): delete heartbeat h_%s, %v, %v", heartbeat.JobId)
...
func (s *JobsManager) DelayOrSkipJob(ctx context.Context, namespace string, objectType string, job v1alpha2.JobData) error {
	...
	if err != nil {
		...
		log.Debugf(" M (Job): heartbeat %s is not found, entry: %+v", key, entry)
	...
	if objectType, ok := event.Metadata["objectType"]; ok {
		...
		case "instance":
			...
			case v1alpha2.JobDelete:
				...
				if err != nil {
					log.Errorf(" M (Job): error reconciling instance %s with delete job action: %s", instanceName, err.Error())
					return err
				} else {
					...
					log.Debugf(" M (Job): delete instance success state entry, instance: %s", instance.ObjectMeta.Name)
		...
		case "target":
			...
			case v1alpha2.JobUpdate:
				_, err = s.apiClient.Reconcile(ctx, deployment, false, namespace, s.user, s.password)
				if err != nil {
					log.Errorf(" M (Job): error reconciling target %s: %s", targetName, err.Error())
					return err
				} else {
			...
			case v1alpha2.JobDelete:
				_, err = s.apiClient.Reconcile(ctx, deployment, true, namespace, s.user, s.password)
				if err != nil {
					log.Errorf(" M (Job): error reconciling instance %s with delete job action: %s", targetName, err.Error())
					return err
				} else {
					err = s.apiClient.DeleteTarget(ctx, targetName, namespace, s.user, s.password)
					if err != nil {
						log.Errorf(" M (Job): failed to target %s: %s", targetName, err.Error())
						return err
					} else {
						log.Debugf(" M (Job): delete target success state entry, target: %s", targetName)
						s.VolatileStateProvider.Delete(ctx, states.DeleteRequest{
							ID: "t_" + targetName,
							Metadata: map[string]interface{}{
								"namespace": namespace,
		...
		case "deployment":
			...
			if job.Action == v1alpha2.JobUpdate {
				_, err = s.apiClient.Reconcile(ctx, *deployment, false, namespace, s.user, s.password)
				if err != nil {
					log.Errorf(" M (Job): error reconciling deployment: %s", err.Error())
					return err
				} else {
					// TODO: how to handle status updates?
					s.VolatileStateProvider.Upsert(ctx, states.UpsertRequest{
			...
			if job.Action == v1alpha2.JobDelete {
				_, err = s.apiClient.Reconcile(ctx, *deployment, true, namespace, s.user, s.password)
				if err != nil {
					log.Errorf(" M (Job): error reconciling deployment with delete job action: %s", err.Error())
					return err
				} else {
					log.Debugf(" M (Job): delete deployment success state entry, deployment: %s", deployment.Instance.ObjectMeta.Name)
					s.VolatileStateProvider.Delete(ctx, states.DeleteRequest{
						ID: "d_" + deployment.Instance.ObjectMeta.Name,
						Metadata: map[string]interface{}{
							"namespace": namespace,

...
func (s *SolutionManager) Reconcile(ctx context.Context, deployment model.DeploymentSpec, remove bool, namespace string, targetName string) (model.SummarySpec, error) {
	...
	for _, step := range plan.Steps {
		...
		case <-ctx.Done():
			...
			err = api_utils.NewAPIError(v1alpha2.UpdateFailed, "Reconciliation was canceled.")
			return summary, err
		default:
			...
			for i := 0; i < retryCount; i++ {
				...
				if stepError == nil {
					...
					if strings.Contains(stepError.Error(), "context canceled") {
			...
			if stepError != nil {
				...
				if strings.Contains(stepError.Error(), "canceled") {
					// TODO: need to handle cancel error by state.
					//       "context canceled."
					// 		 "Reconciliation was canceled."
					err = s.saveSummaryProgress(ctx, deployment.Instance.ObjectMeta.Name, deployment.Generation, deployment.Hash, summary, namespace)
					if err != nil {
						log.ErrorfCtx(ctx, " M (Solution): failed to save summary progress: %+v", err)
						return summary, err

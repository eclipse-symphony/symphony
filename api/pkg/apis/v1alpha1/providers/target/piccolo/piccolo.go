...
func (i *PiccoloTargetProvider) Get(ctx context.Context, deployment model.DeploymentSpec, references []model.ComponentStep) ([]model.ComponentSpec, error) {
	...
	sLog.Infof("  P (Piccolo Target): getting artifacts: %s - %s, traceId: %s", deployment.Instance.Spec.Scope, deployment.Instance.ObjectMeta.Name, span.SpanContext().TraceID().String())

	ret := make([]model.ComponentSpec, 0)
	for _, component := range references {
		name := component.Component.Name
		req, err := http.NewRequest("GET", "http://0.0.0.0:9090/scenario/"+name, nil)
		if err != nil {
			continue
		}
		client := &http.Client{}
...
func (i *PiccoloTargetProvider) Apply(ctx context.Context, deployment model.DeploymentSpec, step model.DeploymentStep, isDryRun bool) (map[string]model.ComponentResultSpec, error) {
	...
	sLog.Infof("  P (Piccolo Target): applying artifacts: %s - %s, traceId: %s", deployment.Instance.Spec.Scope, deployment.Instance.ObjectMeta.Name, span.SpanContext().TraceID().String())
	...
	if err != nil {
		sLog.Errorf("  P (Piccolo Target): failed to validate components: %+v, traceId: %s", err, span.SpanContext().TraceID().String())
		return nil, err
	}
	if isDryRun {
		err = nil
	...
	for _, component := range step.Components {
		if component.Action == model.ComponentUpdate {
			...
			if name == "" {
				...
				sLog.Errorf("  P (Piccolo Target): %+v, traceId: %s", err, span.SpanContext().TraceID().String())
				return ret, err
			}
			resp, err := http.Post("http://0.0.0.0:9090/create-scenario/"+component.Component.Name, "text/plain", nil)
			if err != nil {
				sLog.Errorf("  P (Piccolo Target): fail to create resource")
				return ret, err
			}
			...
			req, err := http.NewRequest("DELETE", "http://0.0.0.0:9090/delete-scenario/"+component.Component.Name, nil)
			if err != nil {
				return ret, err
			}

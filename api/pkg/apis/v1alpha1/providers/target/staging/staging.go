...
func (i *StagingTargetProvider) Get(ctx context.Context, deployment model.DeploymentSpec, references []model.ComponentStep) ([]model.ComponentSpec, error) {
	...
	sLog.Infof("  P (Staging Target): getting artifacts: %s - %s, traceId: %s", deployment.Instance.ObjectMeta.Namespace, deployment.Instance.ObjectMeta.Name, span.SpanContext().TraceID().String())

	var err error
	defer observ_utils.CloseSpanWithError(span, &err)

	namespace := deployment.Instance.ObjectMeta.Namespace
	if namespace == "" {
		namespace = "default"
	}
...
func (i *StagingTargetProvider) Apply(ctx context.Context, deployment model.DeploymentSpec, step model.DeploymentStep, isDryRun bool) (map[string]model.ComponentResultSpec, error) {
	...
	sLog.Infof("  P (Staging Target): applying artifacts: %s - %s, traceId: %s", deployment.Instance.ObjectMeta.Namespace, deployment.Instance.ObjectMeta.Name, span.SpanContext().TraceID().String())

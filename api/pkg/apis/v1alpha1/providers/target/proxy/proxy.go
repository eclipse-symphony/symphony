...
func (i *ProxyUpdateProvider) Get(ctx context.Context, deployment model.DeploymentSpec, references []model.ComponentStep) ([]model.ComponentSpec, error) {
	...
	if err != nil {
		sLog.Error("  P (Proxy Target): failed to get instances: +%v, traceId: %s", err, span.SpanContext().TraceID().String())
		return nil, err
	}
	ret := make([]model.ComponentSpec, 0)
	err = json.Unmarshal(payload, &ret)
	if err != nil {
		sLog.Error("  P (Proxy Target): failed to unmarshall get response: +%v, traceId: %s", err, span.SpanContext().TraceID().String())
		return nil, err
	}
...
func (i *ProxyUpdateProvider) Apply(ctx context.Context, deployment model.DeploymentSpec, step model.DeploymentStep, isDryRun bool) (map[string]model.ComponentResultSpec, error) {
	...
	if len(components) > 0 {
		...
		if err != nil {
			sLog.Error("  P (Proxy Target): failed to post instances: +%v, traceId: %s", err, span.SpanContext().TraceID().String())
			return ret, err
		}
	}
	...
	if len(components) > 0 {
		...
		if err != nil {
			sLog.Error("  P (Proxy Target): failed to delete instances: +%v, traceId: %s", err, span.SpanContext().TraceID().String())
			return ret, err
		}
	}

...
func (i *IoTEdgeTargetProvider) Get(ctx context.Context, deployment model.DeploymentSpec, references []model.ComponentStep) ([]model.ComponentSpec, error) {
	...
	sLog.Infof("  P (IoT Edge Target): getting components: %s - %s, traceId: %s", deployment.Instance.Scope, deployment.Instance.Spec.Name, span.SpanContext().TraceID().String())
...
func (i *IoTEdgeTargetProvider) Apply(ctx context.Context, deployment model.DeploymentSpec, step model.DeploymentStep, isDryRun bool) (map[string]model.ComponentResultSpec, error) {
	...
	if len(modules) > 0 {
		err = i.deployToIoTEdge(ctx, deployment.Instance.Spec.Name, deployment.Instance.Metadata, modules, edgeAgent, edgeHub)
		if err != nil {
	...
	if len(modules) > 0 {
		err = i.remvoefromIoTEdge(ctx, deployment.Instance.Spec.Name, deployment.Instance.Metadata, modules, edgeAgent, edgeHub)
		if err != nil {

...
type CatalogVersionEvalReconciler struct {
	client.Client
...
//+kubebuilder:rbac:groups=federation.symphony,resources=catalogversionevalexpression,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=federation.symphony,resources=catalogversionevalexpression/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=federation.symphony,resources=catalogversionevalexpression/finalizers,verbs=update

func (r *CatalogVersionEvalReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ret ctrl.Result, reterr error) {
	log := ctrllog.FromContext(ctx)
	diagnostic.InfoWithCtx(log, ctx, "Entering the CatalogVersionEvalExpression reconciler")

	evalCR := &federationv1.CatalogVersionEvalExpression{}

	resourceK8SId := evalCR.GetNamespace() + "/" + evalCR.GetName()
	operationName := constants.CatalogVersionEvalOperationNamePrefix
	...
	if err != nil {
		diagnostic.ErrorWithCtx(log, ctx, err, "failed to parse CatalogVersionEvalExpression")
	...
	if CheckNeedtoDelete(evalCR.ObjectMeta) {
		// Delete the CR
		diagnostic.InfoWithCtx(log, ctx, "Deleting the CatalogVersionEvalExpression as it is older than 10 hrs", "CR", req.NamespacedName)
		if err := r.Delete(ctx, evalCR); err != nil {
			// If delete fails, log the error and return (it shall retry)
			diagnostic.ErrorWithCtx(log, ctx, err, "Error deleting the CatalogVersionEvalExpression", "CR", req.NamespacedName)
	...
	if evalCR.DeletionTimestamp.IsZero() {
		...
		if evalCR.Spec.ResourceRef.Name == "" || evalCR.Spec.ResourceRef.Namespace == "" {
			...
			evalCR.Status = federationv1.CatalogVersionEvalExpressionStatus{
		...
		if err := r.Get(ctx, *evalCR.Spec.ResourceRef.GetNamespacedName(), catalogversion); err != nil {
			diagnostic.ErrorWithCtx(log, ctx, err, "Error getting the parent catalogversion resource", "CR", req.NamespacedName)
			status := &federationv1.ActionStatusBase{}
			status.ActionStatus = federationv1.ActionResult{
				Error: &federationv1.ProvisioningError{
		...
		// Send catalogversion spec properties to settings vendor
		properties, err := r.ApiClient.GetParsedCatalogVersionProperties(ctx, evalCR.Spec.ResourceRef.Name, evalCR.Spec.ResourceRef.Namespace, "", "")
		if properties == nil {
...
func (r *CatalogVersionEvalReconciler) SetupWithManager(mgr ctrl.Manager) error {
	// We need to re-able recoverPanic once the behavior is tested #691
	recoverPanic := false
	return ctrl.NewControllerManagedBy(mgr).
		Named("CatalogVersionEval").
		WithOptions((controller.Options{RecoverPanic: &recoverPanic})).
		For(&federationv1.CatalogVersionEvalExpression{}).
		Complete(r)
}

...
func TestCreateResourceGroup(t *testing.T) {
	...
	if tenantId == "" || clientId == "" || clientSecret == "" || resourceGroup == "" || location == "" {
		t.Skip("Skipping because TEST_AZURE_TENANT_ID, TEST_AZURE_CLIENT_ID, TEST_AZURE_CLIENT_SECRET, TEST_AZURE_RESOURCE_GROUP, or TEST_AZURE_LOCATION enviornment variable is not set")
	}

	token, err := GetAzureToken(tenantId, clientId, clientSecret, "https://management.azure.com/.default")
	assert.Nil(t, err)
	assert.NotEqual(t, "", token)
	err = CreateResourceGroup(token, resourceGroup, location)
	assert.Nil(t, err)
}

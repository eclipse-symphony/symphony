...
func (i *RedisPubSubProvider) processMessage(topic string, handler v1alpha2.EventHandler, msg *redis.XMessage) error {
	...
	if shouldRetry {
		internalErr := handler.Handler(topic, evt)
		mLog.ErrorfCtx(i.Ctx, "  P (Redis PubSub) : processing failed with retriable error (%v) for message %s for topic %s, group %s", internalErr, msg.ID, topic, handler.Group)
	...
	mLog.InfofCtx(i.Ctx, "  P (Redis PubSub) : processing succeeded for message %s: (%v) for topic %s, group %s", msg.ID, msg, topic, handler.Group)
	return nil
}
